...
class Image:
    def __init__(self, x_pixels=0, y_pixels=0, num_channels=0, filename=''):
        ...
        if x_pixels and y_pixels and num_channels:  #check the inputs 
            self.x_pixels = x_pixels # size 
            self.y_pixels = y_pixels # size
            self.num_channels = num_channels  #like RGB channels (3 channels)
            self.array = np.zeros((x_pixels, y_pixels, num_channels)) # This numpy array creates a 3-dim matrix of the photo
        elif filename:  # get the size and channels with the help of a helper function
